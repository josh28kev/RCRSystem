...
public class Usuario_Modelo_Tabla extends AbstractTableModel{
    ...
    public Object getValueAt(int row, int col) {
        ...
        switch (a_columnas[col]) {
            case ae_id:
                return usuario.obtener_identificacion();
            case ae_nombre:
                return usuario.obtener_nombre();
            case ae_telefono:
                return usuario.obtener_numero_telefono();
            case ae_permiso:
                if(usuario.obtener_puesto()==1){
                return "Administrador";
                }
                if(usuario.obtener_puesto()==2){
                return "Compras";
                }
                  if(usuario.obtener_puesto()==3){
                return "Ventas";
                }
            default:
                return "";
