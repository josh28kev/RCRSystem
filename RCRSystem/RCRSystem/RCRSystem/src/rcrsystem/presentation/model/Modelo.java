...
public class Modelo extends java.util.Observable {
    ...
    public void revisar_inventario(List<Inventario> rows) {
        for (Inventario inv : rows) {
            if (inv.obtener_cantidad().compareTo(new BigDecimal("19999")) == 1) { //si tiene mas de 20 toneladas está para una venta
                a_reposiciones.add(inv.obtener_material().obtener_nombre());
            }
        }
    }
