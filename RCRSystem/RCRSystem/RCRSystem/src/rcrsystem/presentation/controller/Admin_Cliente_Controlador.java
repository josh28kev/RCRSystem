...
public class Admin_Cliente_Controlador {
    ...
    public void cargar() {
        this.colocar_clientes(Cliente_DAO.obtener_clientes());
    ...
    public void buscar() {
        ...
        a_modelo.obtener_filtro().poner_codigo_c(a_vista.jTextField_filtro.getText());
        List<Cliente> rows = new ArrayList();
        for (Cliente p : this.obtener_clientes()) {
            if (p.obtener_codigo_c().contains(a_modelo.obtener_filtro().obtener_codigo_c())) {
                rows.add(p);
            }
        }
    ...
    public void borrar(int row) {
        try {
            ...
            int ax = JOptionPane.showConfirmDialog(null, "¿Seguro que desea borrar el cliente?", "Confirmación", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
            if (ax == JOptionPane.YES_OPTION) {
                Cliente_DAO.eliminar(seleccionada);
