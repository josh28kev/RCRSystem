...
public class Admin_Proveedores_Controlador {
    ...
    public void cargar() {
        this.colocar_proveedoress(Proveedor_DAO.obtener_proveedores());
    ...
    public void buscar() {
        ...
        a_modelo.obtener_filtro().poner_codigo(a_vista.jTextField_filtro.getText());
        List<Proveedor> rows = new ArrayList();
        for (Proveedor p : this.obtener_proveedoress()) {
            if (p.obtener_codigo().contains(a_modelo.obtener_filtro().obtener_codigo())) {
                rows.add(p);
            }
        }
   ...
   a_vista.jTextField_filtro.setText("");
    ...
    public void borrar(int row) {
        try {
            ...
            int ax = JOptionPane.showConfirmDialog(null, "¿Seguro que desea borrar el proveedor?", "Confirmación", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
            if (ax == JOptionPane.YES_OPTION) {
              Proveedor_DAO.eliminar(seleccionada);
